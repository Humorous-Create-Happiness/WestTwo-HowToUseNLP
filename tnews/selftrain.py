...
with open('../afqmc/234.txt', 'r', encoding='utf-8') as f:  # 与训练时一致
    lines = f.readlines()
    for line in lines:
...
def preprocess_data(data, tokenizer):
    processed_data = []
    for item in data:
        label = int(item["label"])-100           #______________________________________
...
classifier = Classifier(model, output_dim=17)
